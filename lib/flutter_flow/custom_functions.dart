...
List<double>? quanityCart() {
  List<int> extractQuanity(List<dynamic> quanityArray) {
    // Verifica si el array no es nulo ni vacío
    if (quanityArray.isEmpty) {
      return [];
    }

    // Extrae solo los valores de "quanity" de cada objeto en el array
    return quanityArray.map((item) {
      if (item is Map<String, dynamic> && item.containsKey('quanity')) {
        return item['quanity'] as int; // Convierte el valor a int
      } else {
        return 0; // Valor predeterminado si no existe "quanity"
      }
    }).toList();
  }
}

List<double>? generate010() {
